...
OpenGLShader::OpenGLShader(char* shader_file_path , int shader_type , bool* success) 
...
OpenGLShader::OpenGLShader(char* shaderFilePath , int shader_type ,  const std::string& zipFile , bool* success)
...
OpenGLShader::OpenGLShader(int shader_type, const char* shaderSource, bool* success)
	: m_shaderType(shader_type)
{
	if (success != nullptr)
		*success = true;
	...
	if (!wasSuccessful)
	{
		ReportShaderError(m_shaderType, m_shaderID, (const char*)shaderSource, m_shaderFileName, "Embedded Code");
		if (success != nullptr)
			*success = false;
	}
}
...
void OpenGLShader::ReportShaderError(int m_shaderType, int m_shaderID,const char* shaderText, char* m_shaderFileName, char* m_shaderFilePath )
{
