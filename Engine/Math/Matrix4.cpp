...
Matrix4::Matrix4(Vec3f upVector, Vec3f rightVector, Vec3f forwardVector)
{
	m_matrix[0] = upVector.x;	m_matrix[4] = rightVector.x;	m_matrix[8]  = forwardVector.x;	m_matrix[12] = 0.0f;
	m_matrix[1] = upVector.y;	m_matrix[5] = rightVector.y;	m_matrix[9]  = forwardVector.y;	m_matrix[13] = 0.0f;
	m_matrix[2] = upVector.z;	m_matrix[6] = rightVector.z;	m_matrix[10] = forwardVector.z;	m_matrix[14] = 0.0f;
	m_matrix[3] = 0.0f;			m_matrix[7] = 0.0f;				m_matrix[11] = 0.0f;			m_matrix[15] = 1.0f;
}
...
void Matrix4::ApplyMatrix(const Matrix4* mat)
{
	const float* n = mat->m_matrix;
...
void Matrix4::ApplyMatrix(const float* n)
{
	float answer[16];
	answer[0] = m_matrix[0] * n[0] + m_matrix[4] * n[1] + m_matrix[8] * n[2] + m_matrix[12] * n[3];
	answer[1] = m_matrix[1] * n[0] + m_matrix[5] * n[1] + m_matrix[9] * n[2] + m_matrix[13] * n[3];
...
void Matrix4::ApplyRotateDegrees(float degrees,float x,float y,float z)
{
...
void Matrix4::ApplyRotateRadians(float radians, float x, float y, float z)
{
	float _cos = cos(radians);
	float _sin = sin(radians);
...
bool Matrix4::Inverse()
{
	...
	det = m_matrix[0] * inv[0] + m_matrix[1] * inv[4] + m_matrix[2] * inv[8] + m_matrix[3] * inv[12];
...
Vec3f Matrix4::GetTranslation()
{
	Vec3f translation(m_matrix[12], m_matrix[13], m_matrix[14]);
	return translation;
}


Vec3f Matrix4::GetScale()
{
	Vec3f scale(m_matrix[0], m_matrix[5], m_matrix[10]);
	return scale;
}


Vec3f Matrix4::GetEulerDegrees()
{
	Vec3f euler = GetRotationRadians();
	euler.x = radians2degrees(euler.x);
	euler.y = radians2degrees(euler.y);
	euler.z = radians2degrees(euler.z);
	return euler;
}


Vec3f Matrix4::GetRotationRadians()
{
	Vec3f euler;
	euler.x = atan2(m_matrix[6], m_matrix[10]);
	euler.y = atan2(-m_matrix[2], sqrt(m_matrix[6] * m_matrix[6] + m_matrix[10] * m_matrix[10]));
	euler.z = atan2(m_matrix[1], m_matrix[0]);
	return euler;
}


void Matrix4::ApplyVector(Vec3f& vector)
{
	Vec3f temp = vector;
	float w = 1.0f;

	vector.x = m_matrix[0] * temp.x + m_matrix[4] * temp.y + m_matrix[8]  * temp.z + m_matrix[12] * w;
	vector.y = m_matrix[1] * temp.x + m_matrix[5] * temp.y + m_matrix[9]  * temp.z + m_matrix[13] * w;
	vector.z = m_matrix[2] * temp.x + m_matrix[6] * temp.y + m_matrix[10] * temp.z + m_matrix[14] * w;
}
